...
def build_request(args: argparse.Namespace) -> PlanRequest:
    from .user_memory import get_topic_defaults, load_memory
    ...
    # Only this topic's own recent sessions are consulted.
    memory = load_memory(DEFAULT_USER_MEMORY)
    mem_defaults = get_topic_defaults(memory, topic)
...
def collect_pack(
    ...
    for video in current_videos:
        # Match on the fingerprint the history actually stores, with the bare
        # bvid as fallback for entries written before fingerprints existed.
        if (video.fingerprint and video.fingerprint in recent) or (video.bvid and video.bvid in recent):
            removed["recent"] += 1
            continue
    ...
    # Minimum candidate ratio check: warn if eligible < 3× output days
    min_required = request.days * 3
    if len(eligible) < min_required:
        import sys
        print(
            f"[warn] 候选池仅 {len(eligible)} 条，不足输出天数({request.days})的 3 倍({min_required})，"
            f"编排精度可能受限。建议增加关注源或放宽过滤条件。",
            file=sys.stderr,
        )
    stats.update(
        {
...
def cmd_wizard(args: argparse.Namespace) -> int:
    ...
    # Wizard input needs no separate write: cmd_plan below records this session,
    # notes and durations included, into the same rolling log.
...
def _update_memory_after_plan(args: argparse.Namespace, request: "PlanRequest") -> None:
    """Record this session in the rolling per-topic log."""
    from .user_memory import append_episodic, load_memory, save_memory
    memory = load_memory(DEFAULT_USER_MEMORY)
